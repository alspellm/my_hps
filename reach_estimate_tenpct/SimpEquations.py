...
class SimpEquations:
    ...
    @staticmethod
    def rate_Vpi(m_Ap, m_pi, m_V, alpha_dark, f_pi, rho, phi):
        x = m_pi / m_Ap
        y = m_V / m_Ap
        coeff = alpha_dark * SimpEquations.Tv(rho, phi) / (192.0 * math.pow(math.pi, 4))
        return coeff * math.pow((m_Ap / m_pi), 2) * math.pow(m_V / m_pi, 2) * math.pow((m_pi / f_pi), 4) * m_Ap * math.pow(SimpEquations.Beta(x, y), 3 / 2.0)

    @staticmethod
    def br_Vpi(m_Ap, m_pi, m_V, alpha_dark, f_pi, rho, phi):
        rate = SimpEquations.rate_Vpi(m_Ap, m_pi, m_V, alpha_dark, f_pi, rho, phi) + SimpEquations.rate_2pi(m_Ap, m_pi, m_V, alpha_dark)
        if 2 * m_V < m_Ap:
            rate += SimpEquations.rate_2V(m_Ap, m_V, alpha_dark)
        return SimpEquations.rate_Vpi(m_Ap, m_pi, m_V, alpha_dark, f_pi, rho, phi) / rate

    @staticmethod
    def br_2V(m_Ap, m_pi, m_V, alpha_dark, f_pi, rho, phi):
        if 2 * m_V >= m_Ap:
            return 0.0
        rate = (
            SimpEquations.rate_Vpi(m_Ap, m_pi, m_V, alpha_dark, f_pi, rho, phi)
            + SimpEquations.rate_2pi(m_Ap, m_pi, m_V, alpha_dark)
            + SimpEquations.rate_2V(m_Ap, m_V, alpha_dark)
        )
        return SimpEquations.rate_2V(m_Ap, m_V, alpha_dark) / rate
