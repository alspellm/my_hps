...
def make_variables_html(parent_dir, parent_name, name):
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>{parent_name} Page</title>
    </head>
    <body>
        <h1>{parent_name} - {name} </h1>
        <p>Click 1D or 2D plots:</p>
        
        <ul>
            <li><a href="{basename}/plots-1d.html">plots-1d</a></li>
            <li><a href="{basename}/plots-2d.html">plots-2d</a></li>
        </ul>
    </body>
    </html>
        ''')

def make_var_1d_plots(parent_dir, parent_name, name, input_images_dir, keyword):
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>{parent_name} - Variables - 1D Plots As Function of Iteration</title>
    </head>
    <body>
        <h1>Signal and Background Variables for {parent_name}</h1>
    ...
    for var in variables:
        with open(os.path.join(base_dir, f'{var}.html'), 'w') as f:
            ...
            img_divs = "\n".join([
                f'<div><a href="{input_pngs_dir}/{png}" target="_blank"><img src="{input_pngs_dir}/{png}" alt="{png}" width="1000"></a></div>'
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>{parent_name} {var} 1D Plots By Iteration</title>
        <style>
            div {{
...
def make_var_2d_plots(parent_dir, parent_name, name, input_images_dir, keyword):
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>{parent_name} - Variables - 2D Plots As Function of Iteration</title>
    </head>
    <body>
        <h1>Signal and Background Variables for {parent_name}</h1>
        <p> Click a variable to see how Signal and Background change with iterative cuts </p>
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>{parent_name} {var} 2D Plots By Iteration</title>
        <style>
            div {{
                margin-bottom: 20px; /* Add spacing between images */
            }}
...
base_dir = 'zalpha_0pt3_html'
parent_name = 'zalpha_0pt3'

#Parent
make_parent_html(base_dir, parent_name)

#Variables
make_variables_html(base_dir, parent_name, 'variables')

#Variable 1D Plots
input_images_dir = '/sdf/group/hps/users/alspellm/projects/THESIS/ana/zbi_html/zalpha_plots_html_test/1d_plots'
make_var_1d_plots(base_dir, parent_name, 'plots_1D', input_images_dir, "iteration_initial_"):

#Variable 2D Plots
input_images_dir = "/sdf/group/hps/users/alspellm/projects/THESIS/ana/zbi_html/zalpha_plots_html_test/2d_plots"
make_var_2d_plots(base_dir, parent_name, 'plots_2D', input_images_dir, "initial_"):
